...
```bash
pets-stepfunction$ sam build --use-container
```
...
```bash
pets-stepfunction$ sam logs -n StockCheckerFunction --stack-name pets-stepfunction --tail
```
...
```bash
pets-stepfunction$ pip install -r tests/requirements.txt --user
# unit test
pets-stepfunction$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
pets-stepfunction$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name pets-stepfunction
```
